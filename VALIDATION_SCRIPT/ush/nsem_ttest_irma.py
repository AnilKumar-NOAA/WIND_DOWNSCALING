...
# Original Author(s): Andre van der Westhuysen & Anil Kumar
...
# storm peak in order to detect 
   ...
   intercept = float(linreg.intercept_[0])
   ...
   if plotflag:
      ...
      axs[0].set(ylabel=unit, title=label+": p-value = "+"{:.3f}".format(pvalue1))
   ...
   if plotflag:
      plt.clf()
      print('Model is '+label[0:6])
      if label[0:6]=="ADCIRC":
         print('Plotting ADCIRC amplitudes')
         model=abs(model)
         observations=abs(observations)
      maxval = max(max(model), max(observations))
      plt.plot(observations, model, 'ko', markersize=2)
      plt.plot(maxval*np.arange(2), intercept + maxval*np.arange(2)*slope, 'k-')
      plt.plot([0, maxval], [0, maxval], 'k--')
      plt.plot([0, maxval], [0, 0.9*maxval], 'k:')
      plt.plot([0, maxval], [0, 1.1*maxval], 'k:')
...
def test_90_accuracy2(model, model2, observations, plotflag=False, direc="", label="model", unit="Value"):
# Function to perform the 90% accuracy test for FEMA using
# a paired t-test on the model output and observations.
# Data is partitioned into a subset before and after the 
# storm peak in order to detect 
   ...
   obs1 = observations[:cuttime]
   mod1 = model[:cuttime]
   mod21 = model2[:cuttime]
   obs2 = observations[cuttime:]
   mod2 = model[cuttime:]
   mod22 = model2[cuttime:]
   ...
   if plotflag:
      ...
      axs[0].plot(obs1, 'k-o', markerfacecolor='w', markeredgecolor='k', markersize=3, label='Observations')
      axs[0].plot(mod1, 'b-', label='Model')
      axs[0].plot(mod21, 'b--', label='Model (no waves)')
      axs[0].legend()
      axs[0].plot(0.9*obs1, 'k--', linewidth=0.5)
      axs[0].plot(1.1*obs1, 'k--', linewidth=0.5)
      axs[0].plot(0.0*mod1, 'k:', linewidth=0.5)
