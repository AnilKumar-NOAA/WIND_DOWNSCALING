...
# storm peak in order to detect 
   ...
   if plotflag:
      ...
      plt.plot(10.*(mod1-obs1)/np.maximum(obs1,0.01), 'r-', label='d*10')
      print(np.mean((mod1-obs1)/np.maximum(obs1,0.01)))
      plt.plot((10.*np.mean((mod1-obs1)/np.maximum(obs1,0.01))+0.*obs1), 'r--', label='mean(d*10)')
      plt.legend()
   ...
   if plotflag:
      d1 = (mod1-obs1)/np.maximum(obs1,0.01)
      maxdev = max(d1)
      plt.clf()
