...
def test_90_accuracy_single(model, observations, plotflag=False, direc="", label="model", unit="Value"):
...
# storm peak in order to detect 
   ...
   if np.mean(np.abs(mod1-obs1)/obs1) > 0:
   ...
   if plotflag:
      ...
      plt.plot(10.*np.abs(mod1-obs1)/np.maximum(obs1,0.01), 'r-', label='d*10')
      print(np.mean(np.abs(mod1-obs1)/np.maximum(obs1,0.01)))
      plt.plot((10.*np.mean(np.abs(mod1-obs1)/np.maximum(obs1,0.01))+0.*obs1), 'r--', label='mean(d*10)')
      plt.legend()
   ...
   if plotflag:
      d1 = np.abs(mod1-obs1)/np.maximum(obs1,0.01)
      maxdev = max(d1)
      plt.clf()
