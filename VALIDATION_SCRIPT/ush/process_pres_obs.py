...
for station in stations.columns:
    df = pd.read_csv(PARMnsem+'/storms/'+STORM+'/'+str(station)+'h'+runPDY[0:4]+'.txt', skiprows=[1],  usecols=['#YY','MM','DD','hh','mm','WDIR','WSPD','PRES','GST','WTMP','WVHT'],delim_whitespace=True,na_values=[99.00,999.0])
    ...
    df = df.rename(columns={'PRES':station})
    df = df[['Date',station]]
...
print(obs.head(10))
obs.to_csv(COMINobs+"/ObsPres.csv", float_format="%.3f")
