...
obs_gust = dataset[['OBS_PSFC_CM']].fillna(0)
mod_gust = dataset[['MODEL_PSFC_CM']].fillna(0)
...
plt.xlim([900, max(max(model)+5, max(observations)+5)])
plt.ylim([900, max(max(model)+5, max(observations)+5)])
plt.gca().set_aspect('equal', adjustable='box')
plt.text(1.0,  950.0, "Slope="+"{:.3f}".format(slope), fontsize=16)
plt.text(1.0,  930.0, "RMSE="+"{:.4f}".format(rmse), fontsize=16)
plt.text(1.0,  910.0, "SI="+"{:.4f}".format(si), fontsize=16)
plt.title('Cape May, NJ\nStation 8536110\nSurf Psfc (10m) hPa', fontsize=20)
plt.legend()
plt.show()
