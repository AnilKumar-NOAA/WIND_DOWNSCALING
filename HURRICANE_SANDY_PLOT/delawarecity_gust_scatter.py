...
dataset = read_csv('delawarecity_ready.csv', delimiter = ',')
dataset.replace(to_replace=9999, value=np.nan, inplace=True)

obs_gust = dataset[['OBS_GUST_CM']].fillna(0)
mod_gust = dataset[['MODEL_GUST_CM']].fillna(0)
...
plt.text(1.0,  24.0, "Slope="+"{:.3f}".format(slope), fontsize=16)
plt.text(1.0,  22.0, "RMSE="+"{:.4f}".format(rmse), fontsize=16)
plt.text(1.0,  20.0, "SI="+"{:.4f}".format(si), fontsize=16)
plt.title('Delaware city, NJ\nStation 8551762\nWind Gust (10m) m/s', fontsize=20)
plt.legend()
plt.savefig('delawarecity_gust.png')
plt.show()
