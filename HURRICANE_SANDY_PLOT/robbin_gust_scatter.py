...
dataset = read_csv('robbins_reef_ready.csv', delimiter = ',')
dataset.replace(to_replace=9999, value=np.nan, inplace=True)

obs_gust = dataset[['OBS_GUST_RR']].fillna(0)
mod_gust = dataset[['MODEL_GUST_RR']].fillna(0)
...
plt.title('Robbins Reef, NJ\nStation 8530973\nWind Gust (10m) m/s', fontsize=20)
plt.legend()
plt.savefig('robin_gust.png')
plt.show()
